...
if (isset($_POST['submit'])) {
    ...
    if ($password !== $c_password) {
        header("location:view_users.php?password_error");
        exit;
    }
    ...
    if ($ext === 'png' or $ext === 'jpg' or $ext === 'jpeg' or $ext === 'PNG' or $ext === 'JPG' or $ext === 'JPEG') {
        ...
        if ($uploaded) {
            ...
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
                header("location:view_users.php?updated");
            } else {
                header('location:view_users.php?not_updated');
            }
        }
    } else {
        header('location:view_users.php?invalid_extention');
    }
    $stmt = null;
}