...
<div class="contents">
    ...
    <table class="normal_table" cellspacing="0">
        <tr>
            <td colspan="10"><h3 align="center">Earning (Today)</h3></td>
        </tr>
        <tr>
        ...
        $sql = "SELECT * FROM tbl_booking WHERE booking_date BETWEEN '$start_date' AND '$end_date' AND paid=1 AND paid_alert=1";
        ...
        <?php
        $sql = "SELECT SUM(total_price) as total_earned FROM tbl_booking WHERE booking_date BETWEEN '$start_date' AND '$end_date' AND paid=1 AND paid_alert=1";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $total_result = $stmt->fetch(PDO::FETCH_ASSOC);
        //print_r($total_result);
        echo '<tr>';
        echo '<td><strong>Total Earned</strong></td>';
        echo '</tr>';
        echo '<tr>';
        echo '<td>'.$total_result["total_earned"].'</td>';
        echo '</tr>';
        ?>
    </table>
